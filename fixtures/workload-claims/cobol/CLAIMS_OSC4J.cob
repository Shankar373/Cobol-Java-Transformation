       ...
       01  WS-PR-PAY-ID          PIC X(5).
       01  WS-PR-CLAIM-ID        PIC X(4).
       01  WS-PR-PAY-DATE        PIC X(8).
       01  WS-PR-AMOUNT-STR      PIC X(5).
       01  WS-PR-METHOD          PIC X(4).

       01  WS-PT-CLAIM-ID        PIC X(4) OCCURS 100 TIMES.
       01  WS-PT-AMOUNT          PIC 9(5) OCCURS 100 TIMES.

       01  WS-PAY-COUNT          PIC 9(3) VALUE 0.
       01  WS-PAY-IDX            PIC 9(3) VALUE 0.

       01  WS-HEADER             PIC X(80).
       01  WS-SPACER             PIC X(80).
               ...
               NOT AT END
                   ...
                   UNSTRING PAYMENT-REC DELIMITED BY "|"
                       INTO WS-PR-PAY-ID
                            ...
                            WS-PR-METHOD
                   END-UNSTRING
                   MOVE WS-PR-CLAIM-ID TO WS-PT-CLAIM-ID(WS-PAY-COUNT)
                   MOVE WS-PR-AMOUNT-STR TO WS-PT-AMOUNT(WS-PAY-COUNT)
           END-READ
           ...
           READ CLAIMS-FILE
               ...
               NOT AT END
                   ...
                   ELSE
                       ...
                       ELSE
                           IF WS-CLAIM-AMOUNT < 500
                               ...
                               ADD 1 TO WS-REJECTED-COUNT
                           ELSE
                               MOVE 'APPROVED' TO WS-SETTLEMENT-STATUS
                               ADD 1 TO WS-APPROVED-COUNT
                               PERFORM FIND-PAYMENT
                               IF WS-PAY-MATCH-FOUND = 'Y'
                                   IF WS-PAY-MATCH-AMOUNT = WS-CLAIM-AMOUNT
                                       MOVE 'PAID_IN_FULL' TO WS-SETTLEMENT-STATUS
                                       ADD 1 TO WS-PAID-COUNT
                                   ELSE
                                       MOVE 'PARTIAL' TO WS-SETTLEMENT-STATUS
                                       ADD 1 TO WS-PAID-COUNT
                                   END-IF
                               ELSE
                                   MOVE 'UNPAID' TO WS-SETTLEMENT-STATUS
                   ...
                   IF WS-SETTLEMENT-STATUS = 'REJECTED'
                       DISPLAY "REJECT:" WS-CR-CLAIM-ID " "
                           WS-CR-PATIENT-NAME " reason=" WS-CR-STATUS
                           UPON STDERR
                   END-IF
                   IF WS-CLAIM-AMOUNT < 500 AND WS-CR-STATUS = 'A'
                       DISPLAY "LOW_AMOUNT:" WS-CR-CLAIM-ID " "
                           WS-CR-PATIENT-NAME " amt=" WS-CR-AMOUNT-STR
                           UPON STDERR
                   END-IF
           END-READ.
       ...
       FIND-PAYMENT.
           ...
           PERFORM CHECK-PAYMENT VARYING WS-PAY-IDX
               FROM 1 BY 1 UNTIL WS-PAY-IDX > WS-PAY-COUNT
           END-PERFORM.

       CHECK-PAYMENT.
           IF WS-PT-CLAIM-ID(WS-PAY-IDX) = WS-CR-CLAIM-ID
               MOVE 'Y' TO WS-PAY-MATCH-FOUND
               MOVE WS-PT-AMOUNT(WS-PAY-IDX) TO WS-PAY-MATCH-AMOUNT
               ADD WS-PAY-MATCH-AMOUNT TO WS-TOTAL-PAYMENTS
           END-IF.
